...
- If interactive question tool is available in the current mode, use `{{ASK_USER}}`.
...
1. Read the `fix-agent-prompt.md` template from this skill directory (`{{SKILLS_DIR}}/code-review-fix/fix-agent-prompt.md`)
2. Substitute variables:
   ...
   - `{PROJECT_RULES}` — read from {{RULES_SOURCE}}
3. Dispatch subagent:
   ```
