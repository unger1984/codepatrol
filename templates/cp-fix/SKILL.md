...
Use `{{ASK_USER}}` when available. Otherwise ask the same plain-language question in a normal user-facing message
...
When `{{ASK_USER}}` is unavailable, present the proposed groups in a normal user-facing question and wait for an
explicit reply.
