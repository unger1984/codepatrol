...
`/cp-review` сначала проверяет compliance (соответствие дизайну, плану и правилам), затем quality (архитектура, стиль, тесты, безопасность). Отчёт сохраняется как `.review.report.md`. `/cp-fix` исправляет findings в порядке отчёта.
