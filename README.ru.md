...
Workflow-first, spec-driven набор AI-скиллов для [Claude Code](https://docs.anthropic.com/en/docs/claude-code) и [Codex CLI](https://github.com/openai/codex). CodePatrol заменяет одноразовое взаимодействие полным жизненным циклом задачи — от идеи до дизайна, планирования, code review, фиксов и документации — с возобновляемостью сессий и аудит-трейлом. Шаблоны в `templates/`, платформенные значения в `platforms/*.env`.
...
flowchart TD
    ...
    cpfixn2 -- "отложенные findings" --> drafts
    cpidea2 -. "загружает черновик" .-> drafts
...
Отложенные findings могут порождать черновики в `.ai/drafts/` (см. ниже).

### `.ai/drafts/` — черновики отложенных задач

Легковесные наброски задач, сохранённые для будущей проработки. Черновики могут создавать:
- **`/cp-fix` и `/cp-plan-fix`** — когда finding слишком масштабен для фиксера (архитектурное изменение, кросс-модульный рефакторинг)
- **`/cp-idea`** — когда в процессе обсуждения возникает побочная тема, или когда пользователь решает, что идея пока не готова к полноценному дизайну

Черновики хранятся как `.ai/drafts/<YYYY-MM-DD-HHMM>-<slug>.draft.md`. Каждый черновик фиксирует источник, описание проблемы, собранный контекст и подсказки. Перед созданием скилл проверяет, нет ли уже черновика с таким же scope — чтобы не дублировать.

Чтобы превратить черновик в полноценную задачу, передайте его в `/cp-idea`:

```
/cp-idea .ai/drafts/2026-03-07-1500-refactor-auth-middleware.draft.md
```

`/cp-idea` загружает контекст черновика, создаёт задачу и помечает черновик как `processed` со ссылкой на созданную задачу — чтобы не подхватить его повторно.
...
- **Отложенная работа.** Идеи, не готовые к реализации, и findings, слишком масштабные для фиксера, становятся черновиками в `.ai/drafts/`, готовыми к превращению в задачу через `/cp-idea`.
...
Глубина процесса адаптируется под задачу: для мелких правок — минимальная, для крупных фич — полная. В любой момент пользователь может сохранить идею как черновик вместо перехода к дизайну. Перед написанием дизайна `/cp-idea` явно спрашивает: продолжить к дизайну или сохранить в черновик.
...
`/cp-review` сначала проверяет compliance (соответствие дизайну, плану и правилам), затем quality (архитектура, стиль, тесты, безопасность). Отчёт сохраняется как `.review.report.md`. `/cp-fix` исправляет findings в порядке отчёта. Findings, слишком масштабные для фиксера, можно отложить в `.ai/drafts/` для последующей проработки через `/cp-idea`.
...
- `/cp-idea` также принимает путь к черновику: `/cp-idea .ai/drafts/2026-03-07-1500-refactor-auth.draft.md` — загружает контекст и создаёт задачу из черновика.
...
- CodePatrol устанавливает скилл маршрутизации (`using-codepatrol`), который маппит короткие алиасы (`/idea`, `/plan`, `/review`...) и естественно-языковые интенты на нужные команды.
...
📋 Plan: автоматически вызван /cp-plan — создан rate-limiting.plan.md (3 этапа, 2 batch-файла)
   Rules compliance pre-check: пройден

✅ Ревью плана: автоматически вызван /cp-plan-review
   Findings: 1 minor (не указан fallback при недоступности Redis)

🔧 Фикс плана: автоматически вызван /cp-plan-fix
...
Команды, запускающие субагентов (`/cp-idea`, `/cp-plan`, `/cp-plan-review`, `/cp-review`, `/cp-execute`, `/cp-docs`, `/cp-rules`), используют трёхуровневую систему моделей:
...
- `/cp-idea` (research, дизайн) рекомендуется запускать на самой мощной модели — команда предупредит, если текущая модель может быть недостаточной.
...
templates/                        source-of-truth шаблоны скиллов
  _shared/                        переиспользуемые partials для нескольких скиллов
platforms/                        платформенные значения placeholder'ов
skills/                           сгенерированный output (Claude) из ./install.sh build
.claude-plugin/                   манифесты Claude marketplace
.claude/rules/                    правила авторинга скиллов
.ai/docs/                         AI-ориентированная документация проекта
...
Логирование активности workflow отключено по умолчанию. Для включения детального лога в `workflow.md` при отладке и анализе создайте файл-флаг в проекте:

```bash
touch .ai/.enable-log
```

При включённом логировании каждая workflow-задача записывает структурированный лог: вызовы скиллов, результаты субагентов, решения пользователя, отклонения от плана. Предназначено для разработчиков плагина — конечным пользователям не требуется.

Для отключения удалите файл:
...
- **ci.yml** — запускается при push/PR в main: пересобирает `skills/`, проверяет актуальность, валидирует структуру, отклоняет legacy-имена скиллов
- **release.yml** — запускается при тегах версий (`v*`): собирает release-архив и создаёт GitHub release
- **validate-release.yml** — запускается при PR/push/dispatch: валидирует структуру релиза, подстановку placeholder'ов и отклонение legacy
