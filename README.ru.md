...
| **Ad hoc режим** | Нет | Любая команда работает вне workflow-задач; `/cpdocs` принимает запросы на естественном языке |
...
Создаётся и обновляется командой `/cpdocs` — как шаг workflow после завершения задачи, так и через ad hoc запросы на естественном языке.
...
Также работает в ad hoc режиме: `/cpdocs добавь диаграмму потока данных для модуля авторизации` — парсит запрос, исследует кодовую базу и создаёт или обновляет нужный документ.
