...
| **Возобновляемость сессий** | Начинать заново в новой сессии | `/cpresume` восстанавливает точную стадию из артефактов |
...
| **Валидация плана** | Нет выделенного шага | Цикл `/cpplanreview` → `/cpplanfix` перед реализацией |
| **Документация** | Нет выделенного шага | `/cpdocs` — AI-ориентированные docs в `.ai/docs/` с навигацией и валидацией |
| **Эволюция правил** | Нет выделенного шага | `/cprules` — предлагает улучшения правил проекта на основе накопленных findings |
| **Ad hoc режим** | Нет | Любая команда ревью/фикса работает вне workflow-задач |
...
Команды, запускающие субагентов (`/cpreview`, `/cpexecute`), используют трёхуровневую систему моделей:
...
- `/cpatrol` (research, дизайн, планирование) рекомендуется запускать на самой мощной модели — команда предупредит, если текущая модель может быть недостаточной.
