...
2. If multiple plan review reports with `open` findings exist (across `.ai/tasks/` and `.ai/reports/`), list them and ask the user which one to work with. Use `AskUserQuestion` if available.
...
Before starting fixes, read project rules from ``.claude/rules/*.md` and `CLAUDE.md`` to ensure plan fixes remain compliant.

## Bounded Revalidation

Revalidation must confirm that:
- the addressed finding is actually closed
- the updated plan remains consistent with design and project rules from ``.claude/rules/*.md` and `CLAUDE.md``
...
- **Execute in current session** — invoke `/cp-execute` directly (Use the Skill tool to invoke the target skill directly.). Best when context is fresh and task is small.
